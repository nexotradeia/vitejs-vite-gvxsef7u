...
const MOCK_EARNINGS = [
  {ticker:"TSLA",nombre:"Tesla",fecha:"Hoy",fechaEn:"Today",hora:"Tras cierre",eps_est:"$0.51",rev_est:"$22.3B",sorpresa:null},
  {ticker:"MSFT",nombre:"Microsoft",fecha:"Mañana",fechaEn:"Tomorrow",hora:"Tras cierre",eps_est:"$2.82",rev_est:"$60.8B",sorpresa:null},
  {ticker:"GOOGL",nombre:"Alphabet",fecha:"Mañana",fechaEn:"Tomorrow",hora:"Tras cierre",eps_est:"$1.53",rev_est:"$78.6B",sorpresa:null},
  {ticker:"META",nombre:"Meta",fecha:"Miér 17",fechaEn:"Wed 17",hora:"Tras cierre",eps_est:"$4.71",rev_est:"$36.2B",sorpresa:null},
  {ticker:"NFLX",nombre:"Netflix",fecha:"Vie 19",fechaEn:"Fri 19",hora:"Antes apertura",eps_est:"$4.53",rev_est:"$9.7B",sorpresa:"+8%"},
];
const MOCK_TRENDING = [
...
function EarningsPage({lang}){
  return(
    <div>
      ...
      {MOCK_EARNINGS.map(e=>(
        <div key={e.ticker} style={{background:C.surface,border:`1px solid ${e.fecha==="Hoy"?C.gold+"55":C.border}`,borderRadius:14,padding:"14px 18px",marginBottom:10,boxShadow:C.shadow,background:e.fecha==="Hoy"?"#fffbeb":C.surface}}>
          <div style={{display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
            <span style={{background:C.accentDim,color:C.accentText,borderRadius:8,padding:"3px 10px",fontSize:13,fontWeight:800,fontFamily:"monospace"}}>${e.ticker}</span>
            <span style={{fontWeight:700,color:C.text,fontSize:14}}>{e.nombre}</span>
            <span style={{color:e.fecha==="Hoy"?"#b45309":C.muted,fontWeight:e.fecha==="Hoy"?800:500,fontSize:13}}>{e.fecha==="Hoy"?"🔥 HOY":(lang==="en"?e.fechaEn:e.fecha)}</span>
            <span style={{color:C.muted2,fontSize:12}}>{e.hora}</span>
...
function Sidebar({user,following,onFollow,onProfile,onNeedAuth,onAI,lang}){
  const t=LANGS[lang];
  const mini=[{ticker:"BTC",price:"$68,420",change:+4.2},{ticker:"NVDA",price:"$875.40",change:+2.8},{ticker:"TSLA",price:"$172.80",change:-3.1},{ticker:"ETH",price:"$3,820",change:+5.7},{ticker:"AAPL",price:"$189.50",change:+0.4},{ticker:"SPY",price:"$521.30",change:-0.8}];
  return(
    <div>
      {/* AI Widget */}
